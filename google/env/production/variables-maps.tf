...
variable "k8s_options" {
  ...
  default = {
    ...
    enable_dashboard           = true  # Whether the Kubernetes Dashboard is enabled for this cluster
    enable_network_policy      = true  # Whether we should enable the network policy addon for the master. This must be enabled in order to enable network policy for the nodes. It can only be disabled if the nodes already do not have network policies enabled.
    ...
    enable_istio = true # The istion enable for the cluster
  }
}
...
variable "node_options" {
  ...
  default = {
    disk_size    = "10"            # Size of the disk attached to each node, specified in GB. The smallest allowed disk size is 10GB.
...
variable "node_pool_options" {
  ...
  default = {
    ...
    max_pods_per_node     = 20  # The maximum number of pods per node in this node pool. Note this setting is currently in Beta: https://www.terraform.io/docs/providers/google/r/container_node_pool.html#max_pods_per_node
  }
}
